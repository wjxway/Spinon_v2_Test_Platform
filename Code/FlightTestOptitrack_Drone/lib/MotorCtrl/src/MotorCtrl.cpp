...
namespace Motor
{
	namespace
	{
		...
		DRAM_ATTR Circbuffer<Motor_FB_t, Motor_FB_buffer_size> Motor_FB_buffer;
		 ...
		 */
		IRAM_ATTR void Generate_DShot_item(rmt_item32_t *const pointer, const uint32_t throttle) noexcept
		{
		...
		void Keep_alive_task(void *pvParameters)
		{
			...
			constexpr int64_t keep_alive_interval = 60;
		 ...
		 */
		IRAM_ATTR int32_t Ticks_to_val(uint32_t ticks)
		{
		 ...
		 * 		-5 -> CRC error
		 */
		IRAM_ATTR int64_t Parse_DShot_Feedback(volatile rmt_item32_t *RMTptr)
		{
		...
		void IRAM_ATTR DShot_ISR(void *)
		{
			...
			if (intr_st & (1 << (12 + DSHOT_RMT_RX_CHANNEL)))
			{
				...
				// push to buffer
				Motor_FB_buffer.push(Motor_FB_t{{.time = esp_timer_get_time(), .throttle = last_set_throttle, .eRPM = eRPM}});
		...
		IRAM_ATTR void DShot_transmit(uint32_t val)
		{
			in_set_throttle = 1;
